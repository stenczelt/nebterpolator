...
class Molecule(object):
    ...
    def write(self, fnm=None, ftype=None, append=False, selection=None, **kwargs):
        ...
        ## Fill in comments.
        if 'comms' not in self.Data:
            self.comms = ['Generated by ForceBalance from %s: Frame %i of %i' % (fnm, i + 1, self.ns) for i in
                          range(self.ns)]
    ...
    def center_of_mass(self):
        ...
        return np.array(
            [np.sum([xyz[i, :] * PeriodicTable.get(self.elem[i], 0.0) / M for i in range(xyz.shape[0])], axis=0) for xyz
             in self.xyzs])
    ...
    def distance_displacement(self):
        ...
        for sn in range(len(self)):
            if hasattr(self, 'boxes') and pbc:
                drij_i, dxij_i = AtomContact(self.xyzs[sn], AtomIterator,
                                             box=np.array([self.boxes[sn].a, self.boxes[sn].b, self.boxes[sn].c]),
                                             displace=True)
            else:
